...
SCAN_TIMEOUT = 20.0
REC_TIMEOUT  = 360.0  # +60 s over v6.3 to cover inference time
...
class StreamMode:
    ...
    MFCC_LUNG    = "mfcc_lung"
    RESULT_HEART = "result_heart"   # ← new
    RESULT_LUNG  = "result_lung"    # ← new
...
def handle_notification(sender, data: bytearray):
    if _is_data_chunk(data):
        ...
        elif state.mode == StreamMode.RESULT_HEART:   # ← new
            _handle_result_chunk(data, "heart")
        elif state.mode == StreamMode.RESULT_LUNG:    # ← new
...
def _all_done() -> bool:
    """
    Always wait for audio + both MFCCs.
    Result files are optional — if the model is disabled on the firmware
    side (ENABLE_HEART_MODEL=0 / ENABLE_LUNG_MODEL=0) those streams
    never arrive, so we don't block on them.
    We consider results done if either:
      a) we received and closed the stream (hr_done / rr_done), OR
      b) we never got a RESULT_*_START for them (hr_expected_b == 0).
    """
    base_done   = state.audio_done and state.heart_done and state.lung_done
    hr_settled  = state.hr_done or state.hr_expected_b == 0
    rr_settled  = state.rr_done or state.rr_expected_b == 0
    return base_done and hr_settled and rr_settled
