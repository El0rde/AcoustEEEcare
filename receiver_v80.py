#!/usr/bin/env python3
"""
AcoustEEEcare v8.0 — Host-side BLE receiver
============================================================
Matches firmware v8.0 (BLE-only, heart/lung organ selection,
dynamic MTU, no SD card).

Protocol changes vs v6.7
─────────────────────────
• START header now carries the organ tag:
      "START:<audio_bytes>:<HEART|LUNG>\\n"
• MFCC packet sequence field is the FRAME INDEX (u16 LE),
  not a rolling counter.  Gap detection uses the frame idx.
      [frame_idx u16 LE][payload_len u16 LE][float32 * n_coeffs]
• New commands (send before REC):
      "HEART"  — switch pipeline to heart (default)
      "LUNG"   — switch pipeline to lung
• Audio chunk payload size is dynamic (MTU − 3 − 4 header bytes).
  The receiver must not assume a fixed chunk size.

Full pipeline (single mode):
    Host sends : "HEART" or "LUNG"   (optional, default HEART)
    Host sends : "REC"
    Device sends:
        "START:<audio_bytes>:<organ>\\n"
        [seq u16 LE][len u16 LE][int16 payload]   — audio chunks
        "finished\\n"
        "MFCC_START:<n_frames>:<n_coeffs>\\n"
        [frame_idx u16 LE][len u16 LE][float32*n_coeffs]
        "MFCC_END\\n"

Error messages from firmware:
    "ERR:<CODE>\\n"  — e.g. ERR:SAADC, ERR:TIMEOUT, ERR:DSP

Expected MFCC shapes (from trained models):
    HEART  665 frames × 25 coeffs
    LUNG   324 frames × 26 coeffs

Usage:
    pip install bleak numpy
    python receiver_v80.py [--organ heart|lung]
"""
...
class State:
    ...
    def reset(self):
        ...
        self.active_organ    = "HEART"   # v8.0: parsed from START header
        self.audio_done      = False
        self.audio_samples   = bytearray()
        self.audio_seq       = 0         # rolling seq counter (u16)
        self.audio_gaps      = 0
        self.audio_chunks    = 0

        # MFCC stream
        # v8.0: sequence field in MFCC packet is the FRAME INDEX, not a
        # rolling counter.  We track the next expected frame index separately.
        self.mfcc_n_frames   = 0
        self.mfcc_n_coeffs   = 0
        self.mfcc_expected_b = 0
        self.mfcc_done       = False
        self.mfcc_bytes      = bytearray()
        self.mfcc_next_idx   = 0         # next expected frame idx (v8.0)
        self.mfcc_gaps       = 0
        self.mfcc_chunks     = 0
...
def _is_data_chunk(data: bytearray) -> bool:
    ...
    True when the packet looks like a sequenced data chunk:
      bytes[0:2] = seq / frame_idx  (u16 LE)
      bytes[2:4] = payload_len       (u16 LE)
      len(data)  = 4 + payload_len
    Only evaluated while inside AUDIO or MFCC mode.
    """
...
def _handle_text(data: bytearray):
    ...
    # ── v8.0 Audio stream header: "START:<bytes>:<HEART|LUNG>" ──
    if text.startswith("START:"):
        try:
            parts = text.split(":")
            n     = int(parts[1])
            organ = parts[2] if len(parts) >= 3 else "HEART"

            state.expected_bytes   = n
            state.active_organ     = organ.upper()
            ...
            dur = n / 2 / SAMPLE_RATE
            print(f"\n  [AUDIO ] START — organ={state.active_organ} "
                  f"expecting {n} B ({n // 2} samples, {dur:.1f} s)")
    ...
    # ── MFCC stream header ──
        ...
        try:
            ...
            # v8.0: frame index starts at 0 and counts up to nf-1
            state.mfcc_next_idx    = 0
            state.mfcc_gaps        = 0
            state.mfcc_chunks      = 0
            ...
            print(f"\n  [MFCC  ] START — organ={state.active_organ} "
                  f"{nf} frames × {nc} coeffs "
                  f"= {state.mfcc_expected_b} B  (expecting {nf} pkts)")
        except (ValueError, IndexError):
...
def _handle_audio_chunk(data: bytearray):
    ...
    if seq != state.audio_seq:
        ...
        # Audio chunk payload size varies (dynamic MTU) — use received ln
        # as best estimate for each missing chunk.
...
def _handle_mfcc_chunk(data: bytearray):
    """
    v8.0 MFCC packet layout:
      [frame_idx u16 LE][payload_len u16 LE][float32 * n_coeffs]

    The firmware uses the FRAME INDEX (not a rolling counter) in the
    first field.  Gap detection compares against the next expected index.
    Missing frames are zero-padded so the output array stays (n_frames, n_coeffs).
    """
    frame_idx = struct.unpack_from("<H", data, 0)[0]
    ln        = struct.unpack_from("<H", data, 2)[0]
    payload   = data[4: 4 + ln]
    ...
    if frame_idx != state.mfcc_next_idx:
        gap_frames = (frame_idx - state.mfcc_next_idx) & 0xFFFF
        gap_bytes  = gap_frames * ln   # each frame has the same payload size
        print(f"\n  [GAP   ] MFCC frame idx {state.mfcc_next_idx}→{frame_idx} "
              f"({gap_frames} frame(s) missing, ~{gap_bytes} B zero-filled)")
        state.mfcc_bytes        += b"\x00" * gap_bytes
        state.mfcc_gaps         += gap_frames
        pstats.mfcc_pkts_lost   += gap_frames
        pstats.mfcc_bytes_lost  += gap_bytes

    state.mfcc_bytes      += payload
    state.mfcc_next_idx    = (frame_idx + 1) & 0xFFFF
    state.mfcc_chunks     += 1
...
def save_mfcc(mfcc_bytes: bytearray, filename: Path) -> None:
    ...
    # Verify expected shape matches trained model input
    expected_shapes = {"HEART": (665, 25), "LUNG": (324, 26)}
    organ = state.active_organ
    if organ in expected_shapes:
        exp_shape = expected_shapes[organ]
        if arr.shape != exp_shape:
            print(f"  [WARN  ] Shape mismatch: got {arr.shape}, "
                  f"model expects {exp_shape} for {organ}")
        else:
            print(f"  [CHECK ] Shape matches model input {exp_shape} ✓")

    means = arr.mean(axis=0)
    n_show = min(5, state.mfcc_n_coeffs)
    coeff_str = " ".join(f"{means[i]:+.3f}" for i in range(n_show))
    print(f"  [CHECK ] MFCC mean per coeff (c0…c{n_show-1}): {coeff_str}")
...
def _session_complete() -> bool:
    """True when both streams have finished, or on firmware error."""
    if state.error:
        return True
...
def _save_session(rec_num: int) -> None:
    ...
    organ = state.active_organ.lower()

    if state.audio_samples and state.expected_bytes > 0:
        wav_fn = OUTPUT_DIR / f"rec_{rec_num:03d}_{organ}_{ts}.wav"
    ...
    if state.mfcc_bytes and state.mfcc_n_frames > 0:
        npy_fn = OUTPUT_DIR / f"mfcc_{rec_num:03d}_{organ}_{ts}.npy"
...
async def run(default_organ: str):
    ...
    async with BleakClient(device) as client:
        ...
        print()
        print("Commands:")
        print("  ENTER           — send REC (uses current organ)")
        print("  h + ENTER       — switch to HEART, then REC")
        print("  l + ENTER       — switch to LUNG, then REC")
        print("  q + ENTER       — quit")
        print()

        rec_num      = 1
        active_organ = default_organ.upper()

        # Tell the firmware our initial organ choice
        await client.write_gatt_char(
            NUS_RX_CHAR_UUID, active_organ.encode(), response=False
        )
        print(f"  Default organ set to: {active_organ}")
        print()
        ...
        try:
            while True:
                ...
                inp = await asyncio.get_event_loop().run_in_executor(
                    ...
                    lambda: input(
                        f"{sep}\n"
                        f"[Ready #{rec_num}  organ={active_organ}]"
                        f"  ENTER=REC  h=HEART  l=LUNG  q=quit: "
                    )
                )
                ...
                if cmd == "q":
                    ...
                    active_organ = "HEART"
                    await client.write_gatt_char(
                        NUS_RX_CHAR_UUID, b"HEART", response=False
                    )
                    print(f"  Organ switched to: HEART")
                    continue

                elif cmd == "l":
                    active_organ = "LUNG"
                    await client.write_gatt_char(
                        NUS_RX_CHAR_UUID, b"LUNG", response=False
                    )
                    print(f"  Organ switched to: LUNG")
                    continue

                else:
                    # ── REC command ──
                    state.reset()
                    print(f"\nSending REC command (organ={active_organ})…")
                    await client.write_gatt_char(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AcoustEEEcare v8.0 receiver")
    parser.add_argument(
        "--organ",
        choices=["heart", "lung"],
        default="heart",
        help="Default organ pipeline (default: heart)",
    )
    args = parser.parse_args()
    asyncio.run(run(args.organ))