...
static int init_sd_card(void)
{
    ...
    if (err != 0) { LOG_INF("disk_access_init failed: %d\n", err); return -1; }
    ...
    mp.fs_data = &fat_fs;
    err = fs_mount(&mp);
    if (err) { LOG_INF("Error mounting fat_fs [%d]\n", err); return err; }
    ...
    for (uint32_t i = 0; i < TOTAL_AUDIO_SAMPLES; i++) {
        ...
        if (accum_pos >= ACCUM_BUF_SIZE) {
            ...
            if (written < 0) {
                ...
                led_error_flash(led_set_yellow);
                fs_close(&file);
                LOG_INF("Recording done -- %u audio bytes + checksum", total_written);
                bt_nus_send(NULL, "REC:OK", 6);   // <-- notify Python client
                return (int)written;
            }
        ...
        uint32_t wait_us    = (elapsed_us < SAMPLE_PERIOD_US) ? (SAMPLE_PERIOD_US - elapsed_us) : 0;
        k_busy_wait(wait_us);
    }
...
// ======================================================
    ...
    */
    ...
    while (true) {
        k_sleep(K_MSEC(100));
        if (is_connected && mtu_exchanged) {
            if (start_recording) {
                start_recording = false;
                led_set_blue();
                if (record_audio_to_sd() == 0) {
